      ...
      <div style={styles.tpassCard}>
        ...
        <div style={styles.tpassDescBlock}>
          ...
          <ul style={{ margin: 0, paddingLeft: '24px', lineHeight: '1.8' }}>
            <li>
              {tpassResult.normal_cost === tpassResult.freq_monthly_cost ? '🔹' : '❌'} 若無任何優惠，原票價總計：
            ...
            <li>✅ <strong>台鐵常客優惠 ({tpassResult.discount_name})</strong>：每月實際花費約 <strong>NT$ {tpassResult.freq_monthly_cost}</strong></li>
          </ul>
        </div>
        ...
        <div style={styles.tpassWarning}>
          ⚠️ 乘車提醒：TPASS 等定期票卡<strong>嚴禁</strong>搭乘 EMU3000(新自強號)、普悠瑪、太魯閣及觀光列車，違者將依無票乘車規定補收票價並加收 50% 違約金。
        </div>
      </div>
    </div>
  )}
...
const injectStyles = () => {
if (!document.getElementById('ep3-styles')) {
const style = document.createElement('style');
style.id = 'ep3-styles';
style.innerHTML = div[style*="cursor: pointer"]:hover { background-color: #F8FAFC; } button[title="清除"]:hover { color: #475569 !important; background-color: #F1F5F9 !important; };
